  ...
  return (
    ...
    >
      <div className="alert alert-info d-flex align-items-start gap-2 mb-4">
        ...
        <div className="small">
          Cars with an <strong>in-progress</strong> or <strong>scheduled</strong> maintenance record are automatically
          excluded from new bookings until their pending work is completed.
        </div>
      </div>
        ...
        {paged.length === 0 ? (
          <div className="p-4">
            <EmptyState
              ...
              message="Add a maintenance record to keep the fleet healthy."
              action={
          ...
          <>
            <div className="table-responsive">
              <table className="table table-de mb-0 align-middle">
                ...
                <tbody>
                  {paged.map((rec) => {
                    const car = getCarById(rec.carId);
                    const isPending = ['in-progress', 'scheduled'].includes(rec.status);
                    return (
                      <tr key={rec.id}>
                        <td>
                          <span className="fw-semibold">{car?.name || 'Deleted car'}</span>
                          <div className="text-muted small">
                            {formatDate(rec.startDate)}{rec.endDate ? ` → ${formatDate(rec.endDate)}` : ''}
                          </div>
                        </td>
                        <td>
                          {rec.type}
                          {rec.description && (
                            <div className="text-muted small">
                              {rec.description.length > 50 ? `${rec.description.slice(0, 50)}…` : rec.description}
                            </div>
                          )}
                        </td>
